...
- 如果传入 `-PromptKimiApiKey`、`-KimiApiKey`，或当前环境已经有 `KIMI_API_KEY`，自动配置 Kimi CLI API Key 认证，写入无 BOM 的 `~\.kimi\config.toml`，免去首次启动后的网页登录。
- 打印下一步怎么打开 Kimi CLI，以及一段可以直接粘贴给 Kimi 的提示词。
- 默认自动进入 AutoVMware 目录并启动 `kimi --yolo`。如果只想安装不启动，传入 `-SkipKimiLaunch`。
...
安装结束后，脚本会自动进入 AutoVMware 目录并启动 Kimi。脚本打印的完整命令也可以手工复用，例如：
...
```powershell
.\install.ps1 -SkipKimiInstall -SkipKimiLaunch
```
...
安装指定版本时，安装器会直连该版本的 release zip 下载地址，不再先请求 GitHub releases API，避免共享出口 IP 被 API rate limit 阻断。
